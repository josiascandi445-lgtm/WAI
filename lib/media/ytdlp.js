...
/** Caminho exacto do binário ffmpeg — "ffmpeg" do PATH se o vendored não existir. */
...
/**
 * Resolve o ficheiro de cookies a usar (se configurado), por ordem de prioridade:
 *
 *   1. YTDLP_COOKIES_FILE  — caminho directo para um cookies.txt já existente no disco.
 *   2. YTDLP_COOKIES_B64   — conteúdo do cookies.txt em base64 (útil quando não dá
 *                            para montar um ficheiro persistente, ex: Railway).
 *   3. ./cookies.txt       — na raiz do projeto (auto-detectado, sem env var nenhuma).
 *   4. ./lib/cookies.txt   — dentro de lib/ (auto-detectado, sem env var nenhuma).
 *
 * Se nenhuma destas existir, o bot continua a funcionar normalmente (sem
 * cookies) e apenas regista um aviso — nunca bloqueia o download.
 *
 * Nota: propositadamente NÃO se usa --cookies-from-browser, porque o bot
 * corre em Render/Railway (sem navegador instalado/sessão local).
 *
 * Ver README/.env.example para instruções de como gerar o cookies.txt.
 */
const ROOT_DIR           = path.join(__dirname, "../..");
const ROOT_COOKIES_PATH  = path.join(ROOT_DIR, "cookies.txt");
const LIB_COOKIES_PATH   = path.join(__dirname, "..", "cookies.txt"); // lib/cookies.txt
...
export function getCookiesFile() {
  ...
  if (process.env.YTDLP_COOKIES_FILE && fs.existsSync(process.env.YTDLP_COOKIES_FILE)) {
    ...
    console.log(`[ytdlp] cookies: a usar ficheiro em YTDLP_COOKIES_FILE (${cachedCookiesPath})`);
  ...
  if (fs.existsSync(ROOT_COOKIES_PATH)) {
    cachedCookiesPath = ROOT_COOKIES_PATH;
    console.log(`[ytdlp] cookies: ficheiro detectado na raiz do projeto (cookies.txt)`);
    return cachedCookiesPath;
  }

  if (fs.existsSync(LIB_COOKIES_PATH)) {
    cachedCookiesPath = LIB_COOKIES_PATH;
    console.log(`[ytdlp] cookies: ficheiro detectado em lib/cookies.txt`);
    return cachedCookiesPath;
  }

  console.warn(
    "[ytdlp] ⚠️  cookies: nenhum cookies.txt encontrado (raiz, lib/, YTDLP_COOKIES_FILE " +
    "ou YTDLP_COOKIES_B64). A continuar sem cookies — alguns vídeos do YouTube podem " +
    "falhar com bloqueio anti-bot. Ver README para configurar."
  );
