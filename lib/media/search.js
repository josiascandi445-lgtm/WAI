...
export async function searchTikTok(query) {
  ...
  if (res.status === 403) {
    // A tikwm está a bloquear o pedido (provável protecção anti-bot contra
    // o IP do servidor, tal como acontece com o YouTube) — não há forma
    // garantida de contornar isto a partir de código; se persistir, é um
    // bloqueio do lado deles, não um bug local.
    throw new Error("Pesquisa TikTok bloqueada (403) — tenta novamente daqui a pouco");
  }
