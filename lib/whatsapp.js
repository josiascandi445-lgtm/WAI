...
import { handleGroupParticipantsUpdate } from "../handlers/onGroupParticipantsUpdate.js";
...
export async function connectToWhatsApp() {
  ...
  // ─── PARTICIPANTES DO GRUPO (entradas/saídas) ────────────────────
  // Usado para as mensagens de despedida quando alguém sai/é removido.
  sock.ev.on("group-participants.update", async (update) => {
    try {
      await handleGroupParticipantsUpdate(sock, update);
    } catch (err) {
      console.error("[WhatsApp] Erro em group-participants.update:", err.message);
    }
  });
