...
export async function connectToWhatsApp() {
  ...
  sock.ev.on("connection.update", async (update) => {
    ...
    if (connection === "open") {
      ...
      // Aguarda 2s para garantir que o pipeline de encriptação está pronto
  ...
  // FIX PRINCIPAL: registado em CADA sock novo.
  // O Baileys cria um novo EventEmitter a cada connectToWhatsApp().
  // O sock anterior (pré-pairing) e o sock novo (pós-restartRequired)
  // são objectos completamente diferentes — os eventos do sock novo
  // nunca chegavam ao handler registado no sock antigo.
  // Solução: registar sempre no sock actual, usando a flag isReady
  // para filtrar mensagens que chegam antes do sock estar pronto.
